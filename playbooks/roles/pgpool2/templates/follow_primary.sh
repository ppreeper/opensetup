...
REPLUSER={{ REPL_USERNAME }}
PCP_USER={{ PGPOOL_USERNAME }}
PGPOOL_PATH=/usr/bin
PCP_PORT=9898
...
ssh -T -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null postgres@${NEW_PRIMARY_NODE_HOST} -i ~/.ssh/{{SSH_KEYID}} ls /tmp > /dev/null
...
ssh -T -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
  postgres@${NODE_HOST} -i ~/.ssh/{{SSH_KEYID}} ${PGHOME}/bin/pg_ctlcluster {{POSTGRES_VERSION}} {{POSTGRES_CLUSTER}} status
...
if [ $? -eq 0 ]; then
  ...
  ssh -T -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null postgres@${NODE_HOST} -i ~/.ssh/{{SSH_KEYID}} "

  set -o errexit

  ${PGHOME}/bin/pg_ctlcluster -w -m f -D ${NODE_PGDATA} stop
...
EOT
  ...
  ${PGHOME}/bin/pg_ctlcluster -l /dev/null -w -D ${NODE_PGDATA} start
...
EOT
        ...
        ssh -T -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
            postgres@${NODE_HOST} -i ~/.ssh/{{SSH_KEYID}} $PGHOME/bin/pg_ctlcluster -l /dev/null -w -D ${NODE_PGDATA} start
    fi
